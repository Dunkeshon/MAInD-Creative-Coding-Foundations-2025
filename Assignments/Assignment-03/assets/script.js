...
// Sound Management
let selectedEatSound = null;
let selectedDeathSound = null;
let soundsData = {
  eat: [],
  death: []
};

// Sound Manager - keeps track of currently playing audio
...
function playSound(soundUrl) {
  ...
  currentAudio.play().catch(e => console.log('Could not play sound:', e));
}

async function fetchSoundsFromFreesound(query) {
  try {
    const response = await fetch(
      `https://freesound.org/apiv2/search/?query=${encodeURIComponent(query)}&page_size=10&fields=id,name,previews`,
      {
        method: 'GET',
        headers: {
...
async function loadSoundsModal() {
  const soundsModal = document.getElementById("sounds-modal");
  soundsModal.classList.add("show");
  pauseGame();

  // Clear and show loading
  document.getElementById("eat-sounds-grid").innerHTML = '<p style="text-align: center; color: #999;">Loading eat sounds...</p>';
  document.getElementById("death-sounds-grid").innerHTML = '<p style="text-align: center; color: #999;">Loading death sounds...</p>';

  // Fetch sounds
  const eatSounds = await fetchSoundsFromFreesound("eat sound retro");
  const deathSounds = await fetchSoundsFromFreesound("death sound retro");

  soundsData.eat = eatSounds;
  soundsData.death = deathSounds;

  // Render eat sounds
  renderSoundGrid(eatSounds, "eat-sounds-grid", "eat");
  
  // Render death sounds
  renderSoundGrid(deathSounds, "death-sounds-grid", "death");
}

function renderSoundGrid(sounds, gridId, type) {
  const grid = document.getElementById(gridId);
  
  console.log(`Rendering ${type} sounds:`, sounds);
  
  // Filter sounds that have preview URLs
  const soundsWithPreviews = sounds.filter(sound => sound.previews && sound.previews['preview-hq-mp3']);
  
  console.log(`${type} sounds with previews:`, soundsWithPreviews.length);
  
  if (soundsWithPreviews.length === 0) {
    grid.innerHTML = '<p style="text-align: center; color: #999;">No sounds found</p>';
    return;
  }

  grid.innerHTML = soundsWithPreviews.map((sound, index) => {
    const isSelected = (type === "eat" && selectedEatSound?.id === sound.id) ||
                       (type === "death" && selectedDeathSound?.id === sound.id);
    
    return `
      <div class="sound-item ${isSelected ? 'selected' : ''}">
        <input 
          type="radio" 
          id="${type}-sound-${index}" 
          name="${type}-sounds" 
          value="${sound.id}"
          onchange="selectSound('${type}', ${index})"
          ${isSelected ? 'checked' : ''}
        >
        <label for="${type}-sound-${index}">${sound.name}</label>
        <button class="play-btn" onclick="previewSound('${sound.previews['preview-hq-mp3']}')">
          ▶ Preview
        </button>
      </div>
    `;
...
function selectSound(type, index) {
  // Get the filtered sounds array
  const soundsWithPreviews = soundsData[type].filter(s => s.previews && s.previews['preview-hq-mp3']);
  const sound = soundsWithPreviews[index];
  
  if (type === "eat") {
    selectedEatSound = sound;
  } else if (type === "death") {
    selectedDeathSound = sound;
  }
  ...
  const radioBtn = document.querySelector(`input[type="radio"][id="${type}-sound-${index}"]`);
  if (radioBtn) {
  ...
  const soundPrefs = {
    eat: selectedEatSound ? { id: selectedEatSound.id, name: selectedEatSound.name, url: selectedEatSound.previews['preview-hq-mp3'] } : null,
    death: selectedDeathSound ? { id: selectedDeathSound.id, name: selectedDeathSound.name, url: selectedDeathSound.previews['preview-hq-mp3'] } : null
  };
  localStorage.setItem('gameSounds', JSON.stringify(soundPrefs));
}
...
function closeSoundsModal() {
  document.getElementById("sounds-modal").classList.remove("show");
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const savedSounds = localStorage.getItem('gameSounds');
  if (savedSounds) {
    try {
      const prefs = JSON.parse(savedSounds);
      if (prefs.eat) {
        selectedEatSound = { id: prefs.eat.id, name: prefs.eat.name, previews: { 'preview-hq-mp3': prefs.eat.url } };
      }
      if (prefs.death) {
        selectedDeathSound = { id: prefs.death.id, name: prefs.death.name, previews: { 'preview-hq-mp3': prefs.death.url } };
      }
    } catch (e) {
      console.log('Could not load saved sounds:', e);
    }
  }
...
async function gameLoop() {
  ...
  if (isGameOver) {
    ...
    // Play death sound if selected, otherwise use default
    if (selectedDeathSound?.previews?.['preview-hq-mp3']) {
      playSound(selectedDeathSound.previews['preview-hq-mp3']);
    } else {
      playSound("assets/resources/loseSound.wav");
    }
...
function consumeFood(oldRow, oldCol) {
  // Play eat sound if selected, otherwise use default
  if (selectedEatSound?.previews?.['preview-hq-mp3']) {
    playSound(selectedEatSound.previews['preview-hq-mp3']);
  } else {
    playSound("assets/resources/CollectSound.wav");
  }
